...
内部権限ブートストラップ。`create-program`、`create-object`、`analyze-cbo-obj`、`analyze-code`、`analyze-symptom`、`team`、`setup` の Step 0 で自動起動。`.claude/settings.local.json` に MCP 名前空間 (SAP プラグイン、レガシー ADT、Notion、IDE) とファイル操作ツール (`Read`, `Write`, `Edit`, `Glob`, `Grep`, `Agent`) の明示的 allowlist を書き込み、`GetTableContents` / `GetSqlQuery` は意図的に除外することで行単位のデータ抽出が毎回ユーザー承認を要求するようにします。直接呼び出し (`/sc4sap:trust-session`) は親スキルへ誘導する拒否メッセージとともに遮断されます。
